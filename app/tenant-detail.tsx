...
export default function TenantDetailScreen() {
  ...
  const [editRoomsWithBeds,           setEditRoomsWithBeds]           = useState<any[]>([]);
  ...
  useEffect(() => {
    if (editTenantRoom) {
      const roomData = editRoomsWithBeds.find((r: any) => r.room.id === editTenantRoom.id);
      if (roomData) {
        let bedsForRoom: any[] = roomData.availableBeds;
        if (tenant?.roomId === editTenantRoom.id && tenant?.bedId && editTenantBed) {
          if (!bedsForRoom.find((b: any) => b.id === editTenantBed.id)) bedsForRoom = [editTenantBed, ...bedsForRoom];
        }
  ...
  const calculateFinancialSummary = () => {
    ...
    if (tenant?.billingConfig && isActive) {
      try {
  ...
  const openEditTenantModal = async () => {
    ...
    if (tenant.propertyId) {
      try {
        const [selectableRes, allBedsRes] = await Promise.all([
          bedService.getAvailableBedsByProperty(tenant.propertyId),
          bedService.getAllBedsByProperty(tenant.propertyId)
        ]);
        const selectableRooms: any[] = selectableRes.data || [];
        const allRooms: any[]        = allBedsRes.data || [];
        setEditRoomsWithBeds(selectableRooms);
        let currentBed: Bed | null = null;
        if (tenant.bedId && tenant.roomId) {
          const currentRoomAllBeds = allRooms.find((r: any) => r.room.id === tenant.roomId);
          currentBed = currentRoomAllBeds?.availableBeds.find((b: any) => b.id === tenant.bedId) || null;
        }
        ...
        if (tenant.roomId) {
          const selected = selectableRooms.find((r: any) => r.room.id === tenant.roomId) || allRooms.find((r: any) => r.room.id === tenant.roomId);
          if (selected) {
            setEditTenantRoom(selected.room);
            const merged = [...(selected.availableBeds || [])];
            if (currentBed && !merged.some((b: any) => b.id === currentBed!.id)) merged.unshift(currentBed);
  ...
  const handleUpdateTenant = async () => {
    ...
    try {
      ...
      setTimeout(() => openEditBillingModal(), 500);
    } catch (err: any) {
