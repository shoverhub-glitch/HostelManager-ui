import {
  ...
  ApiError,
  // RazorpayCheckoutSession removed
...
// Use environment variable for API URL, fallback to deployed URL if not set
...
export const roomService = {
  ...
  async previewBedCountChange(roomId: string, newBedCount: number): Promise<ApiResponse<{ affectedTenants: string[]; willRelocate: boolean }>> {
    return await request<{ affectedTenants: string[]; willRelocate: boolean }>('POST', '/rooms/preview-bed-change', { room_id: roomId, bed_count: newBedCount }, true) as ApiResponse<{ affectedTenants: string[]; willRelocate: boolean }>;
  },
};

export const bedService = {
  async getBeds(propertyId?: string, roomId?: string): Promise<PaginatedResponse<Bed>> {
    let endpoint = '/beds?';
    const params: string[] = [];
    if (propertyId) params.push(`property_id=${encodeURIComponent(propertyId)}`);
  ...
  async updateBed(id: string, data: Partial<Bed>): Promise<ApiResponse<Bed>> {
  ...
  async getAvailableBedsByProperty(propertyId: string): Promise<PaginatedResponse<any>> {
    return await request<any>('GET', `/beds/available-by-property?property_id=${encodeURIComponent(propertyId)}`, undefined, true) as PaginatedResponse<any>;
  },

  /**
   * Returns all rooms with all beds (not just available beds) for a property.
   * Used in tenant-detail for editing tenant bed/room.
   */
  async getAllBedsByProperty(propertyId: string): Promise<PaginatedResponse<any>> {
    return await request<any>('GET', `/beds/all-by-property?property_id=${encodeURIComponent(propertyId)}`, undefined, true) as PaginatedResponse<any>;
  },
};

export const staffService = {
  async getStaff(propertyId?: string, search?: string, page: number = 1, pageSize: number = 50): Promise<PaginatedResponse<Staff>> {
    let endpoint = '/staff?';
    const params: string[] = [];
    if (propertyId) params.push(`property_id=${encodeURIComponent(propertyId)}`);
    if (search) params.push(`search=${encodeURIComponent(search)}`);
